...
async function loadPokemon(id) {
  try {
    ...
    if (currentPokemonId === id) {
      ...
      window.history.pushState({}, "", `./info.html?id=${id}`);
    }
...
function setTypeBackgroundColor(pokemon) {
  ...
  const mainTypeColor = typeColors[types[0]]; 
  const secondaryTypeColor = types[1] ? typeColors[types[1]] : null; 

  if (!mainTypeColor) {
    console.warn(`Color not defined for type: ${types[0]}`);
    return;
  }
  ...
  if (types.length > 1 && secondaryTypeColor) {
    setElementStyles([typeElements[0]], "backgroundColor", mainTypeColor);
    setElementStyles([typeElements[1]], "backgroundColor", secondaryTypeColor);
  } else {
    setElementStyles([typeElements[0]], "backgroundColor", mainTypeColor);
    if (typeElements[1]) {
      setElementStyles([typeElements[1]], "backgroundColor", mainTypeColor);
    }
  }
  ...
  const rgbaColor = rgbaFromHex(mainTypeColor);
  ...
  styleTag.innerHTML = `
    ...
    .stats-wrap .progress-bar::-webkit-progress-value {
        background-color: ${mainTypeColor};
    }
  `;
